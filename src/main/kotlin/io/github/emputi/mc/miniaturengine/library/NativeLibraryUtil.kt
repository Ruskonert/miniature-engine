...
object NativeLibraryUtil {
    ...
    private var architecture = NativeLibraryUtil.Architecture.UNKNOWN
    ...
    private fun getProcessor(): Processor {
        var processor = NativeLibraryUtil.Processor.UNKNOWN
        var bits: Int
        val arch = System.getProperty("os.arch").toLowerCase()
        if (arch.contains("arm")) {
            processor = NativeLibraryUtil.Processor.ARM
        }
        else if (arch.contains("aarch64")) {
            processor = NativeLibraryUtil.Processor.AARCH_64
        }
            ...
            processor = if (32 == bits) NativeLibraryUtil.Processor.PPC else NativeLibraryUtil.Processor.PPC_64
        }
            ...
            processor = if (32 == bits) NativeLibraryUtil.Processor.INTEL_32 else NativeLibraryUtil.Processor.INTEL_64
        }
        ...
        if (NativeLibraryUtil.Architecture.UNKNOWN == architecture) {
            ...
            if (NativeLibraryUtil.Processor.UNKNOWN != processor) {
                ...
                if (name.contains("nix") || name.contains("nux")) {
                    when (processor) {
                        NativeLibraryUtil.Processor.INTEL_32 -> architecture =
                            NativeLibraryUtil.Architecture.LINUX_32
                        NativeLibraryUtil.Processor.INTEL_64 -> architecture =
                            NativeLibraryUtil.Architecture.LINUX_64
                        NativeLibraryUtil.Processor.ARM -> architecture =
                            NativeLibraryUtil.Architecture.LINUX_ARM
                        NativeLibraryUtil.Processor.AARCH_64 -> architecture =
                            NativeLibraryUtil.Architecture.LINUX_ARM64
                        else -> NativeLibraryUtil.Architecture.UNKNOWN
                    }
                } else if (name.contains("aix")) {
                    if (NativeLibraryUtil.Processor.PPC == processor) {
                        architecture =
                            NativeLibraryUtil.Architecture.AIX_32
                    } else if (NativeLibraryUtil.Processor.PPC_64 == processor) {
                        architecture =
                            NativeLibraryUtil.Architecture.AIX_64
                    }
                    ...
                    when (processor) {
                        ...
                        NativeLibraryUtil.Processor.INTEL_64 -> architecture =
                            NativeLibraryUtil.Architecture.OSX_64
                        NativeLibraryUtil.Processor.PPC -> architecture =
        ...
        else
        {
            try
            {
                ...
                for (libPath in libPaths) {
                    val extracted = jniExtractor.extractJni(
